...
const BeliefTree: React.FC<Props> = ({ onBack, refreshTrigger }) => {
  ...
  const { level, progress } = useMemo(() => {
    // Only count Crystal Balls (Positive) for Tree Level, Ambers don't count towards height
    const positiveCount = entries.filter(e => !e.visuals.isAmber).length;
    
    let lvl = 1;
    if (positiveCount >= LEVEL_THRESHOLDS[2]) lvl = 3;
    else if (positiveCount >= LEVEL_THRESHOLDS[1]) lvl = 2;
    ...
    const percentage = lvl === 3 ? 100 : Math.min(100, Math.max(0, ((positiveCount - base) / (target - base)) * 100));

    return { level: lvl, progress: percentage };
  }, [entries]);
  ...
  const { treeOrnaments, rootOrnaments } = useMemo(() => {
    ...
    const mapped = sortedEntries.map((entry, index) => {
      const position = calculatePosition(
        level, 
        index, 
        sortedEntries.length, 
        entry.visuals?.layoutSeed || 0,
        entry.visuals?.isAmber // Pass flag to calculator
      );
    ...
    return {
      treeOrnaments: mapped.filter(e => !e.visuals.isAmber),
      rootOrnaments: mapped.filter(e => e.visuals.isAmber)
    };
  }, [entries, level]);
    ...
    <div className="relative w-full h-full bg-slate-950 text-white flex flex-col items-center overflow-hidden animate-fade-in">
      ...
      <div className="relative z-50 w-full p-6 flex justify-between items-start">
        ...
        <div className="flex flex-col items-end cursor-pointer" onClick={() => setShowLevelInfo(!showLevelInfo)}>
          ...
          <p className="text-[10px] mt-1 text-slate-500 font-mono">
             {entries.length} Memories ({rootOrnaments.length} Buried)
          </p>
        </div>
      </div>
      ...
      {showLevelInfo && (
        <div className="absolute top-20 right-6 z-50 w-64 bg-slate-900/90 backdrop-blur-xl p-4 rounded-xl border border-white/10 shadow-2xl animate-fade-in-up">
           ...
           <p className="mt-3 text-[10px] text-amber-500 border-t border-white/10 pt-2">
             🪨 琥珀：深埋地下的养分，虽不起眼，却滋养着根系。
           </p>
        </div>
      )}
      ...
      <div className="relative flex-1 w-full flex items-end justify-center pb-0 perspective-[1000px] overflow-hidden">
        ...
        {/* --- ROOTS & SOIL LAYER (New) --- */}
        <div className="absolute bottom-0 w-full h-[15%] bg-gradient-to-t from-stone-900 via-stone-900/80 to-transparent z-10 flex items-end justify-center pointer-events-none">
           <div className="w-full h-full opacity-30" style={{backgroundImage: 'url("https://www.transparenttextures.com/patterns/soil.png")'}}></div>
        </div>

        {/* Tree Visuals (Branches) */}
        {level === 1 && (
          <div className="relative w-40 h-60 flex flex-col items-center justify-end animate-tree-sway origin-bottom mb-20 opacity-80 z-20">
        ...
        {level === 2 && (
          <div className="relative w-80 h-[500px] flex flex-col items-center justify-end animate-tree-sway origin-bottom mb-10 opacity-80 z-20">
        ...
        {level === 3 && (
          <div className="relative w-full max-w-lg h-[90%] flex flex-col items-center justify-end animate-glow-pulse origin-bottom opacity-90 z-20">
        ...
        {/* === CRYSTAL BALLS (Tree Branches) === */}
        <div className="absolute inset-0 w-full h-full pointer-events-none z-30">
           <div className="relative w-full h-full max-w-lg mx-auto">
             {treeOrnaments.map((item, i) => (
                <div
                   key={item.id}
                ...
                >
                  <button
                    ...
                    `}
                    style={{ animationDelay: `${item.delay}ms`, animationDuration: '6s' }}
                  >
                     {/* Standard Crystal Visuals */}
                ...
                >
                  <button
                    ...
                    className={`
                      ...
                      hover:border-amber-500 hover:shadow-[0_0_20px_rgba(245,158,11,0.5)]
                      opacity-80
                    `}
                    style={{ animationDelay: `${item.delay}ms` }}
                  >
                     {/* Amber Texture */}
                     <div className="w-full h-full bg-gradient-to-br from-amber-900 via-stone-900 to-black opacity-90 rounded-[inherit] overflow-hidden">
                        <div className="absolute top-1 left-2 w-2 h-1 bg-white/10 rounded-full blur-[1px]"></div>
                     </div>
                  </button>
                </div>
             ))}
