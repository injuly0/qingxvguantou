...
const MoodInput: React.FC<Props> = ({ mood, onBack }) => {
  ...
  const getContainerBg = () => {
    if (step === InputStep.CRYSTALLIZE) return 'bg-slate-950';
    if (step === InputStep.PREVIEW) return 'bg-slate-900';
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col bg-gradient-to-b ${getContainerBg()} transition-all duration-700 overflow-hidden ${isFadingOut ? 'opacity-0 duration-1000' : 'opacity-100'}`}>
      
      {showPicker ? (
        // === MODAL B: STRENGTH PICKER ===
        // Must use relative/absolute if we want it to sit on top, but since we are conditionally rendering,
        // it just takes the place. We pass onBack so user can exit the entire MoodInput flow.
        <StrengthPicker onComplete={handleTraitsComplete} onBack={onBack} />
      ) : (
        // === MODAL C: MOOD INPUT MAIN CONTENT ===
        // IMPORTANT: Wrapper Div to ensure flex layout works correctly without Fragments
                  ...
                  <div className="flex-1 flex flex-col gap-3 min-h-[140px]">
                    <textarea
                      ...
                      className={`w-full flex-1 p-4 rounded-2xl text-base border transition-all resize-none focus:outline-none focus:ring-2 focus:ring-indigo-300/50 shadow-sm
                        ${isDarkTheme 
                          ? 'bg-white/10 text-white border-white/20 placeholder:text-white/30' 
                    ...
                    <div className="relative z-10 flex items-center gap-3">
                      ...
                      <button 
                        ...
                        className={`p-3 rounded-2xl flex items-center gap-2 transition-all shadow-sm flex-1 justify-center border ${imageFile ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-600 border-indigo-50'}`}
                      >
                        <ImageIcon size={18} />
                        <span className="text-sm font-medium">{imageFile ? "已添加" : "添加图片"}</span>
                      </button>
                      <button className="p-3 rounded-2xl bg-white text-slate-600 flex items-center gap-2 shadow-sm flex-1 justify-center border border-indigo-50">
                        <Mic size={18} />
